...
El boton manual "Buscar tracking nuevo ahora" del tab Fulfillment y el cron corren el mismo `FulfillmentSyncService::sync()`, que llama a Packiyo y JTL una orden a la vez. Con muchas ordenes pendientes esto puede tardar mas que el timeout del navegador o del proxy del hosting, asi que la corrida se detiene sola tras `FULFILLMENT_SYNC_TIME_BUDGET_SECONDS` (20s por defecto). La cola guarda `fulfillment_last_checked_at`: primero revisa las ordenes nunca comprobadas (las mas nuevas primero) y despues rota por el checkpoint mas antiguo. De esta forma, las ordenes viejas sin tracking no bloquean indefinidamente las nuevas y las siguientes corridas continuan avanzando sin aumentar el timeout.

La tabla del tab Fulfillment es una vista operativa de todas las ordenes mapeadas. Antes de encontrar tracking muestra `pending`, `waiting_tracking` o `lookup_failed`; despues muestra cada tracking como `synced`, `already_present`, `shipped_pending` o `failed`, junto con el ultimo chequeo y el error correspondiente. La lectura de Packiyo solicita las relaciones configuradas en `PACKIYO_FULFILLMENT_INCLUDE`; si el ID guardado quedo obsoleto, la app busca por numero e identificadores externos y repara el mapping automaticamente.

Cloudflare devuelve HTTP 530 con codigo 1033 cuando no encuentra un conector `cloudflared` saludable para el tunnel de JTL. La app trata especificamente esa combinacion como una caida de conectividad: detiene la corrida tras el primer caso, no crea un fallo por cada orden y muestra una indicacion para revisar la PC de JTL, el servicio `cloudflared` y el estado del tunnel. Otros HTTP 530 conservan su tratamiento normal.
...
Despues de verificar correctamente la primera orden, el boton `Enviar siguiente batch a Packiyo (max. 10)` procesa las siguientes diez ordenes asignadas y previsualizadas. Antes de agrupar el lote excluye `corrected`, `already_corrected` e `ignored_cancelled`; tambien vuelve a leer cada orden y omite cualquier linea provisional cancelada recientemente.
